...
import {
  ...
  Unlock, CheckCircle, RotateCcw, UserCheck
} from "lucide-react";
...
function TableCell({
  table, status, accentColor, onTap,
}: {
  table: TableDef;
  status: TableStatus;
  accentColor: string;
  ...
  const hint =
    status === "available" ? "Tap → Occupied" :
    status === "occupied"  ? "Tap → Free" :
    "Tap → Action";
...
function ReservedActionPanel({ tableLabel, hallName, onOccupy, onFree, onClose }: ActionPanelProps) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-5 pb-2">
          ...
          <p className="text-base font-serif text-[#fca5a5]">{tableLabel} · {hallName}</p>
          <p className="text-xs text-[#f5f5f0]/40 mt-1">Customer has a reservation. What would you like to do?</p>
        </div>
...
export default function TableLayout({ staffMode, onExitStaffMode, statuses, setStatuses }: Props) {
  ...
  const handleTap = (id: string, label: string, hallName: string) => {
    ...
    if (cur === "available") {
      ...
      setStatuses(s => ({ ...s, [id]: "available" as TableStatus }));
  ...
  const handleFree = () => {
    if (!actionTable) return;
    setStatuses(s => ({ ...s, [actionTable.id]: "available" as TableStatus }));
    setActionTable(null);
  };

  const resetAll = () => {
    setStatuses(s => Object.fromEntries(Object.keys(s).map(id => [id, "available" as TableStatus])));
  };

  const occupyAll = () => {
    setStatuses(s => Object.fromEntries(Object.keys(s).map(id => [id, "occupied" as TableStatus])));
  };
      ...
      <AnimatePresence>
        {actionTable && (
          <ReservedActionPanel
            ...
            tableLabel={actionTable.label}
            hallName={actionTable.hallName}
        ...
        <div className="container mx-auto px-6 relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <p className="text-[#f5f5f0]/40 text-sm mt-4 max-w-md mx-auto">
              Tap any table to change its status. Reserved tables show action options. Only staff can free or release tables.
            </p>
            <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
              ...
              <ShieldCheck size={11} />
              Staff Controls Active · Live from Cloud
                ...
                <AnimatePresence>
                  {legend && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                      ...
                      <div className="px-6 py-3 flex flex-col gap-2 text-[10px] tracking-wide text-[#f5f5f0]/50"
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>Available — tap to mark Occupied</span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span>Occupied — tap to free (mark Available)</span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span>Reserved — tap to seat customer or cancel reservation</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            ...
            <button
              onClick={resetAll}
              className="flex items-center gap-2 px-6 py-2.5 text-[10px] tracking-[0.25em] uppercase border transition-all duration-200 hover:scale-105"
              style={{ border: "1px solid rgba(74,222,128,0.25)", color: "#4ade80", background: "rgba(74,222,128,0.05)" }}>
              <CheckCircle size={13} />
              Reset All — Free Every Table
            </button>
            <button
              onClick={occupyAll}
              className="flex items-center gap-2 px-6 py-2.5 text-[10px] tracking-[0.25em] uppercase border transition-all duration-200 hover:scale-105"
